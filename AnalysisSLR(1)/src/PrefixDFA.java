			...
			for (Entry<ArrayList<String>, Integer> entry : mst) {
                if (entry.getValue() == i){         // 找到第i个序号对应的状态
                    ...
                    for (String grammar : end){
                        ...
                        // 如果是带@并且最后为 . ，则在 $ 处 acc
                        if (grammar.contains("@") && grammar.endsWith(".")){
                        ...
                        // 如果文法的点在最后，或者点后面跟着空，说明要规约
                            ...
                            String[] splitStr = grammar.split("->");
                            Character noTerItem = splitStr[0].charAt(0);
                            String dropPoint = grammar.replace(".", ""); // 去掉点
                            int index = newGrammarArray.indexOf(dropPoint); // 在文法中找到去掉点后的文法位置
                            TreeSet<Character> follows = followGather.get(noTerItem);        // 得到非终结符的 Follow 集
                            Object[] followStr = follows.toArray();
                            for (int m = 0; m < followStr.length; m++){        // 只在 Follow 位置处规约
                                for (int j = 1; j < length; j++){
                                    if (slrTable[0][j].equals(followStr[m].toString())){
